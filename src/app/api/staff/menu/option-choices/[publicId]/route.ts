...
import { optionChoiceUpdateSchema } from "@/src/lib/validators";
...
function mapError(err: unknown, where: string): NextResponse {
  if (err instanceof MenuAdminError) {
    const status = err.code === "CHOICE_NOT_FOUND" ? 404 : 422;
...
export async function PATCH(
  ...
  const parsed = optionChoiceUpdateSchema.safeParse(raw);
  if (!parsed.success) {
  ...
  try {
    await updateOptionChoice(auth.staff.restaurantId, publicId, parsed.data, {
      publicId: auth.staff.publicId,
      clientIp: extractClientIp(request),
    });
    return NextResponse.json({ ok: true });
  } catch (err) {
    return mapError(err, "PATCH /api/staff/menu/option-choices/[publicId]");
  }
}
...
export async function DELETE(
  ...
  try {
    await deleteOptionChoice(auth.staff.restaurantId, publicId, {
      publicId: auth.staff.publicId,
      clientIp: extractClientIp(request),
    });
    return NextResponse.json({ ok: true });
  } catch (err) {
    return mapError(err, "DELETE /api/staff/menu/option-choices/[publicId]");
  }
}
