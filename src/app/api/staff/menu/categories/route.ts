...
import { createMenuCategory, MenuAdminError } from "@/src/lib/menuAdmin";
import { menuCategoryCreateSchema } from "@/src/lib/validators";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const { slug } = await createMenuCategory(staff.restaurantId, parsed.data, {
      publicId: staff.publicId,
      clientIp: extractClientIp(request),
    });
    return NextResponse.json({ category: { slug } }, { status: 201 });
  } catch (err) {
    if (err instanceof MenuAdminError) {
      const status = err.code === "SLUG_TAKEN" ? 409 : 422;
      return NextResponse.json({ error: err.message, code: err.code }, { status });
    }
    console.error("[POST /api/staff/menu/categories] failed:", err);
